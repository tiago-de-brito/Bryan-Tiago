...
export const getAnuncio = async (anuncioId: string): Promise<Anuncio | null> => {
  const anuncioRef = doc(db, 'anuncio', anuncioId);
  const anuncioDoc = await getDoc(anuncioRef);

  if (anuncioDoc.exists()) {
    const anuncioData = { id: anuncioDoc.id, ...anuncioDoc.data() } as Anuncio;

    // Verificar se userId está presente
    if (anuncioData.userId) {
      const userRef = doc(db, 'users', anuncioData.userId);
      const userDoc = await getDoc(userRef);

      if (userDoc.exists()) {
        const userData = userDoc.data();
        // Adicionar informações de contato do usuário ao objeto de anúncio
        return { ...anuncioData, ...userData };
      }
    }
...
export const registerUserData = async (userId: string, name: string, telefone: string, estado: string) => {
  try {
    const userRef = doc(db, 'users', userId);
    await setDoc(userRef, { name, telefone, estado });
